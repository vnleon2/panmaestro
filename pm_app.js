...
function showTab(id) {
  ...
  const labels = {
    ...
    'pg-etiquetas':'ETIQUETAS','pg-mercadeo':'MERCADEO'
  };
  ...
  const inits = {
    ...
    'pg-costeo':  () => { _sbCosteoCargar().then(() => { cvMostrar('cv-lista'); recRender(); }); const el = document.getElementById('mro-margen-default'); if(el) el.value = G.margenDefault||40; },
    'pg-hid':     () => { hidCalc(); },
