...
// Repuebla el selector de cultivo (subrecetas categoría pan_mm).
function premPoblarCultivos() {
  const sel = document.getElementById('prem-mm-sub');
  if (!sel) return;
  const cultivos = (_sbRecLista() || []).filter(r => r.cat === 'pan_mm');
  sel.innerHTML = cultivos.length
    ? cultivos.map(r => `<option value="${r.code}">${r.code} · ${r.name}</option>`).join('')
    : '<option value="">Sin recetas categoría pan_mm</option>';
}
...
function premRender() {
  ...
  const cultivoCode = tipoKey === 'mm' ? (document.getElementById('prem-mm-sub').value || null) : null;
  ...
  // "agua", y en modo mm se elimina cualquier línea de levadura.
  ...
  const restoLines = c.lines.filter(l => !l.isAddon).map(l => {
    if (tipoKey === 'mm' && (l.name || '').toLowerCase().includes('levadura')) return null;
    if (l.flour) {
